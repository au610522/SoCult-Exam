...
participant_id.addField('Participant 1:')
participant_id.addField('Participant 2:')
...
if participant_id.OK:
    ...
    Participant_1 = participant_id.data[2]                     #saves data from dialogue box into the variable 'Condiion'
    Participant_2 = participant_id.data[3]                     #saves data from dialogue box into the variable 'Condition'

else:
    core.quit()

columnss = ['Group', 'Word', 'Validity', 'Condition','Participant_1','Participant_2']
indexs = np.arange(0)
...
while True:
    ...
    if key[0] == 'return':
        DATA = DATA.append({
            ...
            'Participant_2': Participant_2}, ignore_index=True)
        DATA.to_csv('data2/WORDS_'+Group+'.csv')
        input = ''
